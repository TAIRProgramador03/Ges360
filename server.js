...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        // Se guarda en la carpeta "public/pdf" relativa al directorio actual
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const connection = await odbc.connect(`DSN=${dbConfig.DSN};UID=${dbUser};PWD=${password};System=${dbConfig.system};charset=utf8`);
...
app.get("/validarArchivo", (req, res) => {
    ...
    if (!nombreArchivo) {
        return res.status(400).json({ existe: false, mensaje: "No se proporcionó el nombre del archivo" });
    }
...
app.get('/clientes', async (req, res) => {
    try {
        ...
        const result = await connection.query("SELECT DISTINCT A.IDCLI, B.CLINOM FROM SPEED400AT.PO_OPERACIONES A INNER JOIN SPEED400AT.TCLIE B ON A.IDCLI=B.CLICVE WHERE A.ID<>86 AND B.CLINOM <> '*** ANULADO ***' ORDER BY CLINOM ASC");
...
app.get('/modelos', async (req, res) => {
    try {
        // Conexión a DB2 con los valores globales de dbUser y password
        const connection = await odbc.connect(`DSN=${dbConfig.DSN};UID=${globalDbUser};PWD=${globalPassword};CCSID=1208`);
        const result = await connection.query("SELECT ID, TRIM(DESCRIPCION) AS MODELO FROM SPEED400AT.PO_MODELO GROUP BY ID, DESCRIPCION ORDER BY TRIM(DESCRIPCION) ASC");
...
app.get('/leasing', async (req, res) => {
    try {
        // Conexión a DB2 con los valores globales de dbUser y password
        const connection = await odbc.connect(`DSN=${dbConfig.DSN};UID=${globalDbUser};PWD=${globalPassword};CCSID=1208`);
        const result = await connection.query("SELECT ID, NRO_LEASING FROM SPEED400AT.TBL_LEASING_CAB ORDER BY NRO_LEASING ASC");
        ...
        res.status(500).json({ success: false, message: 'Error al obtener leasing' });
    }
});

// Iniciar servidor http://192.168.5.125/
app.listen(port, () => {
    console.log(`Servidor corriendo en https://192.168.5.125:${port}`);
});
...
app.get('/tablaContrato', async (req, res) => {
    ...
    // Validación inicial
    if (!idCli || !id) {
        return res.status(400).json({ success: false, message: 'Los parámetros idCli e id son obligatorios.' });
    }
...
app.get('/tablaVehiculo', async (req, res) => {
    ...
    try {
        ...
        // Consulta los contratos asociados al cliente
        const query = `SELECT A.ID, A.CODINI AS CODINI, A.NUMPLA AS PLACA, C.DESCRIPCION AS MARCA, B.DESCRIPCION AS MODELO, B.DESMODGEN AS GENERICO, D.DESCRIP AS TERRENO FROM SPEED400AT.PO_VEHICULO A LEFT JOIN SPEED400AT.PO_MODELO B ON A.IDMOD=B.ID AND A.IDMODGEN=B.IDMODGEN LEFT JOIN SPEED400AT.PO_MARCA C ON A.IDMAR=C.ID LEFT JOIN SPEED400AT.PO_TERRENO D ON A.TP_TRABAJO=D.TPTRA LEFT JOIN SPEED400AT.TBL_LEASING_DET E ON A.ID=E.ID_VEH WHERE A.INIVAL1='0' AND E.ID_VEH IS NULL ORDER BY A.ID DESC`;
        const result = await connection.query(query);

        const cleanedResult = result.map(row => {
            return {
                ID: row.ID !== null && row.ID !== undefined ? row.ID.toString().trim() : null,
                CODINI: row.CODINI !== null && row.CODINI !== undefined ? decodeString(row.CODINI.toString().trim()) : null,
                PLACA: row.PLACA !== null && row.PLACA !== undefined ? decodeString(row.PLACA.toString().trim()) : null,
                MARCA: row.MARCA !== null && row.MARCA !== undefined ? decodeString(row.MARCA.toString().trim()) : null,
                MODELO: row.MODELO !== null && row.MODELO !== undefined ? row.MODELO.toString().trim() : null,
                GENERICO: row.GENERICO !== null && row.GENERICO !== undefined ? decodeString(row.GENERICO.toString().trim()) : null,
                TERRENO: row.TERRENO !== null && row.TERRENO !== undefined ? decodeString(row.TERRENO.toString().trim()) : null,
            };
        });
...
app.post('/insertarContrato', async (req, res) => {
    ...
    let nombreArchivo = `http://192.168.5.125/tair-web/public/pdf/${archivoPdf}`;

    let connection;
    try {
        connection = await odbc.connect(`DSN=${dbConfig.DSN};UID=${globalDbUser};PWD=${globalPassword};CCSID=1208`);
        ...
        const result = await connection.query(queryCabecera, [
            idCliente, nroContrato, cantVehiculos, fechaFormatoDB, duracion,
            kmAdicional, kmTotal, vehSup, vehSev, vehSoc, vehCiu,
            contratoEspecial, tipoCliente, tipoMoneda, story, nombreArchivo, claseContra
        ]);

        const idContratoCab = result.insertId || await obtenerUltimoId(connection);
...
async function obtenerUltimoId(connection) {
    const result = await connection.query(`SELECT MAX(ID) AS ID FROM SPEED400AT.TBLCONTRATO_CAB`);
...
app.post('/insertarDocumento', async (req, res) => {
    ...
    try {
        ...
        console.log("Valores para queryCabecera:", [
            idCliente, idContrato, tipoContrato, nroContrato, vehiculo, fechaFormatoDB, duracion, kmAdicional,
            kmTotal, vehSup, vehSev, vehSoc, vehCiu, Especial, motivo, story
        ]);
        ...
        console.log(queryCabecera, [
            idCliente, idContrato, tipoContrato, nroContrato, vehiculo, fechaFormatoDB, duracion, kmAdicional,
            kmTotal, vehSup, vehSev, vehSoc, vehCiu, Especial, story, nombreArchivo, claseDocu, motivo
        ]);
        const result = await connection.query(queryCabecera, [
            idCliente, idContrato, tipoContrato, nroContrato, vehiculo, fechaFormatoDB, duracion, kmAdicional,
            kmTotal, vehSup, vehSev, vehSoc, vehCiu, Especial, story, nombreArchivo, claseDocu, motivo
        ...
        const idDocumentoCab = result.insertId || await obtenerUltimoIdDoc(connection);
        ...
        res.status(500).json({ success: false, message: 'Error al insertar documento' });
    } finally {
        if (connection) {
            await connection.close();
        }
...
async function obtenerUltimoIdDoc(connection) {
    const result = await connection.query(`SELECT MAX(ID) AS ID FROM SPEED400AT.TBLDOCUMENTO_CAB`);
    return result.length > 0 ? result[0].ID : null;
}
...
app.get('/contratoDetalle', async (req, res) => {
    ...
    if (!contratoId) {
        return res.status(400).json({ success: false, message: 'El contratoId es obligatorio' });
    }
...
app.get('/contContrato', async (req, res) => {
    try {
        // Conexión a DB2 con los valores globales de dbUser y password
        const connection = await odbc.connect(`DSN=${dbConfig.DSN};UID=${globalDbUser};PWD=${globalPassword};CCSID=1208`);
        const result = await connection.query("SELECT COUNT(DISTINCT A.ID) AS PADRE, SUM(CASE WHEN B.TIPO_DOC = 1 THEN 1 ELSE 0 END) AS TIPO_1, SUM(CASE WHEN B.TIPO_DOC = 2 THEN 1 ELSE 0 END) AS TIPO_2, SUM(CASE WHEN B.TIPO_DOC = 3 THEN 1 ELSE 0 END) AS TIPO_3 FROM SPEED400AT.TBLCONTRATO_CAB A FULL OUTER JOIN SPEED400AT.TBLDOCUMENTO_CAB B ON A.ID=B.ID_PADRE");
...
app.get('/tablaconVehiculo', async (req, res) => {
    try {
        ...
        const cleanedResult = result.map(row => {
            return {
                MODELO: row.MODELO !== null && row.MODELO !== undefined
                    ? decodeString(row.MODELO.toString().trim())
                    : null,
                PRECIO_VEH: row.PRECIO_VEH !== null && row.PRECIO_VEH !== undefined ? row.PRECIO_VEH.toString().trim() : null
            };
        });
...
app.post('/insertaLeasing', async (req, res) =>{
    ...
    try {
        ...
        const result = await connection.query(queryCabecera, [
            idCliente, nroLeasing, banco, cantVehiculos, fechaIniDB,
            fechaFinDB, periGracia, nombreArchivo, funcionNumerica(idContrato),
            funcionParteVar(idContrato)
        ]);

        const idLeasingCab = result.insertId || await obtenerUltimoIdLea(connection);
        ...
        res.status(500).json({ success: false, message: 'Error al insertar Leasing' });
    } finally {
        if (connection) {
            await connection.close();
        }
...
async function obtenerUltimoIdLea(connection) {
    const result = await connection.query(`SELECT MAX(ID) AS ID FROM SPEED400AT.TBL_LEASING_CAB`);
    return result.length > 0 ? result[0].ID : null;
}
...
app.get('/consultaVehiculoLeasing', async (req, res) => {
    ...
    try {
        ...
        const query = `
            ...
            WHERE (A.ID = '${id}' OR A.ID_CLIENTE = '${id}') AND E.VEHICULO IS NULL GROUP BY A.CODINI, A.PLACA, TRIM(D.DESCRIPCION), TRIM(A.MODELO), A.NRO_LEASING ORDER BY TRIM(D.DESCRIPCION), TRIM(A.MODELO), A.PLACA`;
...
app.get('/operacionesAsig', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Error al obtener las operaciones' });
    }
});
...
app.post('/insertaAsignacion', async (req, res) =>{
    ...
    try {
        ...
        const result = await connection.query(queryCabecera, [
            idCliente, converFecha, globalDbUser
        ]);

        const idAsignaCab = result.insertId || await obtenerUltimoIdAsigna(connection);
        ...
        res.status(500).json({ success: false, message: 'Error al insertar la asignacion vehicular' });
    } finally {
        if (connection) {
            await connection.close();
        }
...
async function obtenerUltimoIdAsigna(connection) {
    const result = await connection.query(`SELECT MAX(ID) AS ID FROM SPEED400AT.TBL_ASIGNACION_CAB`);
    return result.length > 0 ? result[0].ID : null;
}
...
app.post('/validaContratoCantidad', async (req, res) => {
    ...
    if (!Array.isArray(detalles) || detalles.length === 0) {
        return res.status(400).json({ success: false, mensaje: 'Detalles faltantes o vacíos' });
    }
    ...
    for (let d of detalles) {
        ...
        if (!clase || !idContrato) {
            return res.status(400).json({ success: false, mensaje: `Formato de contrato inválido en el detalle: ${d.idContrato}` });
        }
    ...
    try {
        ...
        for (let key in agrupados) {
            ...
            if (!result || result.length === 0) {
                await connection.close();
                return res.status(404).json({ success: false, mensaje: `Contrato no encontrado: ${clase}_${idContrato}` });
            }
            ...
            let validaciones = [
                { tipo: 'SUPERFICIE', cod: '0', maximo: row.VEH_SUP, actual: row.SUPERFICIE },
            ...
            for (let v of validaciones) {
                ...
                console.log(`Tipo: ${v.tipo}, Actual: ${v.actual}, Nuevos: ${nuevos}, Máximo: ${v.maximo}`);
        ...
        return res.status(500).json({ success: false, mensaje: 'Error interno del servidor durante la validación' });
    }
});