...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function agregarFila() {
        ...
        const lastRowIndex = Array.from(tabla.querySelectorAll('tbody tr')).indexOf(lastRow);
    
        const nuevaFila = document.createElement('tr');
        nuevaFila.innerHTML = `
            <td><input type="text" name="item[]" value="${lastRowIndex + 2}" disabled></td>
            <td>
            ...
            <td><input type="text" name="duracion[]" class="tooltip-input"  value="0" ${checkbox.checked ? '' : 'disabled'} data-tooltip="Duracion contrato"></td>
        ...
        $(nuevaFila).find(".modelo-select").select2({
    ...
    tabla.addEventListener('click', function(e) {
        ...
        if (e.target.closest('tr') && e.target.closest('tr') === tabla.querySelector('tbody tr:last-child')) {
            agregarFila();
        }
    });
...
async function cargarModelos() {
    try {
        const response = await fetch('http://192.168.5.125:3000/modelos'); // Ruta del servidor
        if (!response.ok) {
            throw new Error('Error en la solicitud');
        }
...
async function guardarContrato() {
    ...
    for (let i = 0; i < formData.cantVehiculos.length; i++) {
        if (formData.cantVehiculos[i] < 10) {  
            ...
            mostrarNotificacion('La cantidad es inválido, solo debe contener números', '#C70039');
            return;
        }
    };
    ...
    for (let i = 0; i < formData.duracion.length; i++) {
        if (formData.duracion[i] < 10) {  
            ...
            mostrarNotificacion('La duración es inválido, solo debe contener números', '#C70039');
            return;
        }
    };
    ...
    for (let i = 0; i < formData.kmTotal.length; i++) {
        if (formData.kmTotal[i] < 10) {  
            ...
            mostrarNotificacion('El km total es inválido, solo debe contener números', '#C70039');
            return;
        }
    };
    ...
    for (let i = 0; i < formData.vehSup.length; i++) {
        if (formData.vehSup[i] < 10) {  
            ...
            mostrarNotificacion('valor inválido, solo debe contener números enteros', '#C70039');
            return;
        }
    };
    ...
    if (Number(formData.cantVehiculos) === (Number(formData.vehCiu) + Number(formData.vehSev) + Number(formData.vehSoc) + Number(formData.vehSup))) {
        console.log("Conforme Dr. Fili");
    } else {
        mostrarNotificacion('Total de vehiculos no coincide con la cantidad de vehiculos', '#C70039');
        return;
    }

    // Validación de campos obligatorios
    if (!formData.idCliente || !formData.nroContrato || !formData.cantVehiculos || !formData.fechaFirma || !formData.tipoMoneda || !formData.vehSup || !formData.vehSoc || !formData.vehSev || !formData.vehCiu) {
    ...
    try {
        detalles = Array.from(document.querySelectorAll("#contratos-tbody tr")).map((fila, index) => {
            let modelo = fila.querySelector('select[name="tipo_modelo[]"]').value;
            let tipoTerreno = fila.querySelector('select[name="tipo_terreno[]"]').value;
            let tarifa = Number(fila.querySelector('input[name="tarifa[]"]').value) || 0;
            let cpk = Number(fila.querySelector('input[name="cpk[]"]').value) || 0;
            let rm = Number(fila.querySelector('input[name="rm[]"]').value) || 0;
            let cantidad = Number(fila.querySelector('input[name="cantidad[]"]').value) || 0;
            let duracion = Number(fila.querySelector('input[name="duracion[]"]').value) || 0;
            let compraVeh = Number(fila.querySelector('input[name="compra_veh[]"]').value) || 0;
            let precioVeh = Number(fila.querySelector('input[name="precio_veh[]"]').value) || 0;
            ...
            if (!Number.isInteger(cantidad) || cantidad < 0) {
                throw new Error('Cantidad inválida, solo debe contener números enteros');
            }
            if (!Number.isInteger(duracion) || duracion < 0) {
                throw new Error('Duración inválida, solo debe contener números enteros');
            }
    ...
    try {
        const response = await fetch('http://192.168.5.125:3000/insertarContrato', {
            method: 'POST',
