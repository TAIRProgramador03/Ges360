...
async function cargarContrato() {
    document.getElementById('combo-cliente').addEventListener('change', async function () {
...
async function guardarDocumento() {
    ...
    if (Number(formData.vehiculo) === (Number(formData.vehCiu) + Number(formData.vehSev) + Number(formData.vehSoc) + Number(formData.vehSup))) {
    ...
    try {
        const response = await fetch('http://192.168.5.125:3000/insertarDocumento', {
            method: 'POST',
