document.addEventListener('DOMContentLoaded', () => {
    ...
    if (buscadorVehi) {
        buscadorVehi.addEventListener('input', function () {
            const filtro = this.value.toLowerCase();
            const filas = document.querySelectorAll('.tabla-form-vehi table tbody tr');
...
async function cargartablaClienteLeas() {
    document.getElementById('openModalCli').addEventListener('click', async function () {
        try {
            const response = await fetch(`http://192.168.5.125:3000/tablaClienteLeas`);
            ...
            contratos.forEach((cliente, index) => {
                ...
                row.innerHTML = `
                    <td class="icono-seleccion" data-id="${cliente.IDCLI}" data-nombre="${cliente.CLINOM}">
            ...
            try {
                ...
                alert('Error al obtener los datos de OpenAI. Inténtelo de nuevo más tarde.');
            }
    ...
    filas.forEach(fila => {
        fila.addEventListener('click', function () {
            ...
            if (this.classList.contains('seleccionado')) {
                ...
                const seleccionado = document.querySelector('.icono-seleccion.seleccionado');
...
async function cargartablaVehiculo() {
    document.getElementById('openModal').addEventListener('click', async function () {
        ...
        try {
            ...
            const response = await fetch(`http://192.168.5.125:3000/tablaVehiculo`);
            const contratos = await response.json();
            ...
            contratos.forEach((tablaVehiculo, index) => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td class="icono-seleccion" data-id="${tablaVehiculo.ID}" data-nombre="${tablaVehiculo.PLACA}">
                        <i class="fas fa-check-circle" style="color: green; font-size: 22px;"></i>
                    </td>
                    <td>${tablaVehiculo.CODINI}</td> <!-- Número de contrato -->
                    <td>${tablaVehiculo.PLACA}</td> <!-- Número de contrato -->
                    <td>${tablaVehiculo.MARCA || 'Sin marca'}</td> <!-- Fecha de firma -->
                    <td>${tablaVehiculo.MODELO || 'Sin modelo'}</td> <!-- Periodo -->
                    <td>${tablaVehiculo.GENERICO || 'Sin generico'}</td> <!-- Cantidad total -->
                    <td>${tablaVehiculo.TERRENO || 'Sin terreno'}</td> <!-- Cantidad total -->
                `;
                tbody.appendChild(row);
            });
            ...
            mostrarNotificacion('Error al obtener los datos. Inténtelo de nuevo más tarde.', '#C70039');
        }
    });
}
    ...
    filas.forEach(fila => {
        fila.addEventListener('click', function () {
            ...
            if (this.classList.contains('seleccionado')) {
                // Quitar selección
                this.classList.remove('seleccionado');
                this.querySelector('i').classList.replace('fa-times-circle', 'fa-check-circle');
                this.querySelector('i').style.color = 'green';

                // Eliminar la fila de la tabla de seleccionados
                document.querySelectorAll('#tablaSeleccionados tbody tr').forEach(row => {
                ...
                // Agregar selección
                this.classList.add('seleccionado');
                this.querySelector('i').classList.replace('fa-check-circle', 'fa-times-circle');
                this.querySelector('i').style.color = 'red';

                if (![...tablaSeleccionados.children].some(row => row.dataset.id === placaID)) {
                    const nuevaFila = document.createElement('tr');
                    nuevaFila.dataset.id = placaID;
                    const contador = 0;

                    nuevaFila.innerHTML = `
                        <td><input type="text" name="item[]" value="${contador + 1}" disabled></td>
            ...
            localStorage.setItem('vehiculosSeleccionados', JSON.stringify(seleccionados));
        });
    });
}
...
function cargarSeleccionados() {
    ...
    vehiculos.forEach((vehiculo, index) => {
        ...
        nuevaFila.innerHTML = `
            <td><input type="text" name="item[]" value="${index + 1}" disabled></td>
            <td><input type="text" name="id[]" value="${vehiculo.id}" disabled></td>
            <td><input type="text" name="modelo[]" value="${vehiculo.modelo}" disabled></td>
            <td><input type="text" name="tipo_terreno[]" value="${vehiculo.terreno}" disabled></td>
            <td><input type="text" name="placa[]" value="${vehiculo.placa}" disabled></td>
            <td><input type="text" name="codini[]" value="${vehiculo.codini}" disabled></td>
            <td><input type="number" name="cantidad[]" value="1" disabled></td>
        `;
...
function restaurarSeleccionVehi() {
    const seleccionados = JSON.parse(localStorage.getItem('vehiculosSeleccionados')) || [];
    seleccionados.forEach(({ id, modelo, terreno }) => {
        const icono = document.querySelector(`.icono-seleccion[data-id="${id}"] i`);
        if (icono) {
            icono.classList.replace('fa-check-circle', 'fa-times-circle');
            icono.style.color = 'red';
            document.querySelector(`.icono-seleccion[data-id="${id}"]`).classList.add('seleccionado');
        }
    });
    actualizarContador();
}
...
async function guardaLeasing() {
    ...
    if (!formData.fechaIni || !formData.fechaFin) {
        ...
        if (fechaFinal <= fechaInicio) {
            mostrarNotificacion('La fecha de finalización debe ser mayor que la fecha de inicio', '#C70039');
            return;
        } else {
    ...
    for (let i = 0; i < formData.cantVehiculos.length; i++) {
        if (formData.cantVehiculos[i] < 10) {
            ...
            mostrarNotificacion('La cantidad de vehiculo no es inválido, solo debe contener números', '#C70039');
            return;
        }
    };
    ...
    for (let i = 0; i < formData.periGracia.length; i++) {
        if (formData.periGracia[i] < 10) {
            ...
            mostrarNotificacion('el periodo de gracia no es inválido, solo debe contener números', '#C70039');
            return;
        }
    };

    // Validación de campos obligatorios
    if (!formData.idCliente || !formData.nroLeasing || !formData.banco || !formData.cantVehiculos || !formData.fechaIni || !formData.fechaFin || !formData.idContrato) {
        mostrarNotificacion('Por favor, completa todos los campos obligatorios.', '#C70039');
        return;
    }

    let conta = 0;
    // Obtener detalles de contratos
    const detalles = Array.from(document.querySelectorAll("#contratos-tbody tr")).map((fila, index) => {
        let modelo = fila.querySelector('input[name="modelo[]"]').value;
        let tipoTerreno = fila.querySelector('input[name="tipo_terreno[]"]').value;
    ...
    if (formData.cantVehiculos == conta) {
        ...
        mostrarNotificacion('Debe coincidir la cantidad de vehiculos con los vehiculos seleccionados.', '#C70039');
        return;
    }
    ...
    try {
        const response = await fetch('http://192.168.5.125:3000/insertaLeasing', {
            method: 'POST',
        ...
        const mensaje = error?.odbcErrors?.[0]?.message || error.message || 'Error desconocido';
...
async function subirArchivo(archivo) {
    ...
    try {
        const response = await fetch("http://192.168.5.125:3000/subirArchivo", {
            method: "POST",
            body: formData,
        });
...
async function validarArchivo(nombreArchivo) {
    try {
        const response = await fetch(`http://192.168.5.125:3000/validarArchivo?nombre=${nombreArchivo}`);
        const result = await response.json();
...
function limpiarCampos() {
    ...
    document.querySelectorAll('.icono-seleccion').forEach(icono => {
        ...
        icono.querySelector('i').classList.replace('fa-times-circle', 'fa-check-circle');
...
async function cargarContratosPorCliente(idCli) {
    ...
    if (!idCli) {
        comboContrato.innerHTML = '<option value="">Seleccione un contrato</option>';
        return;
    }

    try {
        const response = await fetch(`http://192.168.5.125:3000/contratosNroAdi?idCli=${idCli}`);
        const contratos = await response.json();

        if (contratos.length === 0) {
            comboContrato.innerHTML = '<option value="">No hay contratos disponibles</option>';
            return;
        }
