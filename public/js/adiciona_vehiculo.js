document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('#combo-box').addEventListener('change', () => limpiarSelect('#combo-box-leasing'));
    document.querySelector('#combo-box-leasing').addEventListener('change', () => limpiarSelect('#combo-box'));
    cargarClientes();
    cargarLeasing();
    cargarClientesAsig();
...
async function cargarLeasing() {
    try {
        const response = await fetch('http://192.168.5.125:3000/leasing'); // Ruta relativa al servidor
        if (!response.ok) throw new Error('Error en la solicitud');

        const leasing = await response.json();
        const comboBox2 = document.querySelector('#combo-box-leasing[name="opciones"]');
...
async function listaVehiculosAsignables() {
    ...
    try {
        const response = await fetch(`http://192.168.5.125:3000/consultaVehiculoLeasing?id=${id}`);
        ...
        vehiLeasing.data.forEach((vehi, index) => {
            ...
            row.innerHTML = `
                <td>${contador = contador + 1} &nbsp;&nbsp;<input type="checkbox" name="item[]" value=""></td>
                <td><input type="text" name="codini[]" value="${vehi.codini}" disabled></td>
                <td><input type="text" name="placa[]" value="${vehi.placa}" disabled></td>
                <td><input type="text" name="marca[]" value="${vehi.marca}" disabled></td>
                <td><input type="text" name="modelo[]" value="${vehi.modelo}" disabled></td>
                <td><input type="text" name="leasing[]" value="${vehi.nro_leasing}" disabled></td>
...
async function cargarOperaciones() {
    document.getElementById('combo-box-asig').addEventListener('change', async function () {
        ...
        try {
            ...
            const response = await fetch(`http://192.168.5.125:3000/operacionesAsig?idCli=${idCli}`);
            const operaciones = await response.json();

            // Si no hay operaciones, limpiar todos los selects
            if (operaciones.length === 0) {
                document.querySelectorAll('.combo-operacion').forEach(select => {
                    select.innerHTML = '<option value="">No hay operaciones disponibles</option>';
                });
                return;
            }
    ...
    document.getElementById('combo-box-asig').addEventListener('change', async function () {
        ...
        try {
            ...
            if (contratos.length === 0) {
                document.querySelectorAll('.combo-contrato').forEach(select => {
                    select.innerHTML = '<option value="">No hay contratos disponibles</option>';
                });
                return;
            }
...
async function guardaAsignacion() {
    ...
    let formData = {
        idCliente: document.querySelector("#combo-box-asig").value.Array,
    ...
    if (!validacionResult.success) {
        mostrarNotificacion(validacionResult.mensaje || 'Validación fallida', '#C70039');
        return false;
    }
    ...
    try {
        const response = await fetch('http://192.168.5.125:3000/insertaAsignacion', {
            method: 'POST',
...
const validarAsignacion = async (detalles) => {
    ...
    const validacionResponse = await fetch('http://192.168.5.125:3000/validaContratoCantidad', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
